...
public class BasicLevelMetrics {
    ...
    ConceptLattice cl;
    ArrayList<ArrayList<Integer>> attributesExtent = null; //список объектов у признаков
    ArrayList<ArrayList<Integer>> objectsIntent = null; //список признаков у объетов
    ArrayList<Integer> attributes = null; //все признаки контекста
    double[][] objectsSimilarityJ = null;
    double[][] objectsSimilaritySMC = null; //матрицы схожести объектов
    ...
    public double simSMC(ArrayList<Integer> intent1, ArrayList<Integer> intent2) {
        ...
        fnlst.removeAll(ListUtils.union(intent1, intent2)); ////  тут есть повторения!!!!!!!!!!!!!!!!
        int fn = fnlst.size();
    ...
    public double avgCohSMC(FormalConcept c) {
        ...
        if (c.extent.size() == 0)
            ...
            return (c.extent.size() > 1) ? 2. * sum / c.extent.size() / (c.extent.size() - 1) : 0;// не 2, так как суммируем дважды
        }
    }
